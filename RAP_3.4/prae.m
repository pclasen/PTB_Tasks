function [results] = prae(subjectNumber,subjectName,startBlock,skipPrac)
  ...
  elseif nargin < 4 || isempty(skipPrac)
      skip = true; % default set to skip practice (use prae_prescan)
  elseif nargin == 4
      skip = skipPrac;
  end
...
%% PRELIMINARY SETUP
  ...
  if skip == true
      msg = sprintf([...
          'We are ready to begin the activity! Earlier, you practiced this task outside of the scanner.\n\n' ...
          'If you have any questions about the activity please ask your experimenter now, by squeezing\n\n' ...
  ...
  if fMRI
    msg = sprintf('The screen will go blank for 10 sec before the first trial starts.');
  else
    msg = sprintf('Press any button to start');
  end
  prae_drawMessage(window,msg);
  Screen('Flip',window);
  WaitSecs(2);

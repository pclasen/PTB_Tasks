function [results] = repref_phase1_long(subjectNumber, subjectName, startBlock, screen, cbhand, cbfaces, cbscenes, skipPrac)
  %-----------------------------------------------------------------------%
  % [results] = repref_phase1(...
  %
  % SUBJNUM    : unique # based on date string (e.g., '101220')
  % SUBJNAME   : unique subject name (e.g., 'jlp')
  % STARTBLOCK : 1|2 - refers to run
  % SCREEN     : 0|1
  ...
  %
  % e.g.,
  % >> results = repref_phase1('101220','jlp',1,0,1,1,1)
  %-----------------------------------------------------------------------%
  ...
  responses = zeros(runBlocks, stims.trials_perblock);
  responseRTs = zeros(runBlocks, stims.trials_perblock);
  acc = zeros(runBlocks, stims.trials_perblock);
  
  trial_durations = zeros(runBlocks,stims.trials_perblock);
  trial_onsets = zeros(runBlocks,stims.trials_perblock);
  delay_onsets = zeros(runBlocks,stims.trials_perblock);
